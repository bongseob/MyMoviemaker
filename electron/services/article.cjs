...
`;
    ...
    지자체 및 배포처에서 배포한 보도자료를 읽고 인터넷신문 기사 형식으로 재작성하라.
...
function registerArticleIpc({ ipcMain, app, isDev }) {
    ...
    ipcMain.handle('process-article', async (event, payload) => {
        try {
            ...
            const articleType = typeof payload === 'string' ? 'gov' : (payload?.articleType || 'gov');
            const articleText = assertText(rawText, 'Article text', 50000);
